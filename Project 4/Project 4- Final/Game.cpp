...
bool sequenceAction(tHand &hand, tGame & game, tDeck & cards) {
	...
	while (key != Exit) {
		...
		if (card != NOTHING) {//error
			cards.cards_sequence[cards.numCards] = card;//modificated
			cards.numCards++;
		}
		key = readKey();
	...
	while (i < cards.numCards && !isMatchEnd(game) && ok) {
		ok = moveTurtle(game, cards.cards_sequence[i]);//modificated
		if (!ok) {
	...
	if (!ok) {
		...
		for (int j = i - 1; j < cards.numCards; j++) {
			includeCard(game.players[game.Actualturn].playshands, cards.cards_sequence[j]);//modificated
			cards.cards_sequence[j] = NOTHING;//modificated
		}
		for (int j = 0; j < i - 1; j++) {
			insert(game.players[game.Actualturn].deck, cards.cards_sequence[j]);//modificated
			cards.cards_sequence[j] = NOTHING;//modificated
		}
	}
